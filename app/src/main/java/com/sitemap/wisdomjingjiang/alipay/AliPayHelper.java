...
public class AliPayHelper {
	...
	// 商户私钥，pkcs8格式
	private static String RSA_PRIVATE = AliPayConfig.getRSA_PRIVATE();
	// 订单交易成功，异步通知地址
	...
	public final String GOODS_NAME = "智慧靖江";
	// 用户id
	private String userID;
	private String express;//运费
	 ...
	 *            订单运费
	 */
	public void pay(String orderID, String goodsName, String goodsInfo,
			String price,String express) {
//		price="0.01";
		this.userID = orderID;
		...
		if (!TextUtils.isEmpty(price)) {
			...
			try {
				...
				e.printStackTrace();
				ShowContentUtils.showLongToastMessage(context, "充值金额错误！");
				return;
			}
			if (money > 0) {

			} else {
				ShowContentUtils.showLongToastMessage(context, "充值金额不能小于0！");
				return;
			}

		} else {
			ShowContentUtils.showLongToastMessage(context, "充值金额不能为空！");
			return;
		}
		// 订单
		...
		try {
			...
			e.printStackTrace();
			ShowContentUtils.showShortToastMessage(context, "RSA签名错误！");
			return;
		}
		// 完整的符合支付宝参数规范的订单信息
	...
	public String getOrderInfo(String subject, String body, String price) {
		...
		orderInfo += "&body=" + "\"" + body + "$*$" + userID + "$*$" + express +"\"";
